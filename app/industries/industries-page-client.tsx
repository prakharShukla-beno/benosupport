...
import type { IndustriesPageData, IndustriesSimpleCard } from "@/sanity/lib/queries"

gsap.registerPlugin(ScrollTrigger)

// Maps a Sanity icon name string back to the actual icon component.
// Keep in sync with ICON_OPTIONS in sanity/schemaTypes/industriesPageType.ts
const ICON_MAP: Record<string, LucideIcon> = {
  Code2, TrendingDown, ShieldAlert, Layers, BarChart2, Users2,
  Bot, ShieldCheck, Cloud, Database, Monitor, Boxes, Cpu, Star, Globe,
}

// Resolves a list of cards, preferring Sanity data if present, else the
// original hardcoded fallback list (which already has real icon components).
function resolveCards<T extends { icon: LucideIcon; title: string; desc: string }>(
  sanityCards: IndustriesSimpleCard[] | undefined,
  fallback: T[],
): T[] {
  if (!sanityCards || sanityCards.length === 0) return fallback
  return sanityCards.map((c) => ({
    icon: ICON_MAP[c.iconName] ?? Code2,
    title: c.title,
    desc: c.description,
  })) as T[]
}
...
function ChallengesSection({ data }: { data: typeof DEFAULT_CHALLENGES_DATA }) {
...
function SolutionsSection({ data }: { data: typeof DEFAULT_SOLUTIONS_DATA }) {
...
function WhyChooseSection({ data }: { data: typeof DEFAULT_WHY_DATA }) {
  const sectionRef = useRef<HTMLDivElement>(null)
  const headingRef = useRef<HTMLDivElement>(null)
  const cardsRef   = useRef<HTMLDivElement>(null)
...
function IndustriesCTA({
  ...
  data: { title: string; paragraph: string; primaryLabel: string; secondaryLabel: string }
}) {
...
export function IndustriesPageClient({
  ...
  industriesData?: IndustriesPageData
}) {
  // Resolve each section's data: Sanity value if present, else the original
  // hardcoded fallback — so the page never breaks or shows blank content.
  const challenges = {
    sectionLabel: industriesData?.challengesSectionLabel || DEFAULT_CHALLENGES_DATA.sectionLabel,
    title: industriesData?.challengesTitle || DEFAULT_CHALLENGES_DATA.title,
    subtitle: industriesData?.challengesSubtitle || DEFAULT_CHALLENGES_DATA.subtitle,
    cards: resolveCards(industriesData?.challengesCards, DEFAULT_CHALLENGES_DATA.cards),
  }

  const solutions = {
    sectionLabel: industriesData?.solutionsSectionLabel || DEFAULT_SOLUTIONS_DATA.sectionLabel,
    title: industriesData?.solutionsTitle || DEFAULT_SOLUTIONS_DATA.title,
    subtitle: industriesData?.solutionsSubtitle || DEFAULT_SOLUTIONS_DATA.subtitle,
    cards: resolveCards(industriesData?.solutionsCards, DEFAULT_SOLUTIONS_DATA.cards),
  }
  ...
  const faq = {
    sectionLabel: industriesData?.faqSectionLabel || "FAQ",
    title: industriesData?.faqTitle || "Frequently Asked Questions",
    subtitle:
      industriesData?.faqSubtitle ||
      "Common questions about our industry solutions and engagement models.",
    items: industriesData?.faq?.length ? industriesData.faq : DEFAULT_FAQ_DATA,
  }

  const cta = {
    title: industriesData?.ctaTitle || "Ready to Transform Your Industry Operations?",
    paragraph:
      industriesData?.ctaParagraph ||
    ...
    primaryLabel: industriesData?.ctaPrimaryLabel || "Request a Proposal",
    secondaryLabel: industriesData?.ctaSecondaryLabel || "Talk To Our Experts",
  }
  ...
  return (
    <div className="min-h-screen bg-background">
          <SiteHeader />
          <main>
      <IndustriesHero heroData={industriesData} />
      <ChallengesSection data={challenges} />
      <SolutionsSection data={solutions} />
      <IndustriesGrid />
      {/* <UseCasesSection /> */}
      <BusinessOutcomes outcomesData={industriesData} />
      <TechStackSection />
      <WhyChooseSection data={whyChoose} />
      <IndustriesFAQ data={faq} />
      <IndustriesCTA data={cta} />
          </main>
          <SiteFooter />
        </div>
